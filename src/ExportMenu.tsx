...
export default function ExportMenu({
  ...
  const allTransactions: Transaction[] = data
    ? [
        ...
        const dayA = new Date(a.timestamp).setHours(0, 0, 0, 0);
        const dayB = new Date(b.timestamp).setHours(0, 0, 0, 0);

        if (dayA !== dayB) {
          return dayA - dayB; // Sort by day ascending
        }

        if (a.type !== b.type) {
          return a.type === "expense" ? -1 : 1; // Expenses before deposits
        }

        return a.timestamp - b.timestamp; // Same day, same type, sort by time ascending
      })
    : [];
  ...
  const exportToCSV = (type: "all" | "expenses" | "deposits") => {
    ...
    if (allTransactions.length > 0) {
      const firstTransactionDate = new Date(
        allTransactions[allTransactions.length - 1].timestamp,
      );
...
`;
      owners.forEach((owner) => {
        ...
        if (balance) {
          const totalToPay =
            balance.expenses + balance.sharedExpenses - balance.deposits;
...
`;
          ...
          if (owner.name === "Robena" && robenaSharedExpense > 0) {
            whatsappText += `  Shared Expenses (2/3): ${robenaSharedExpense.toFixed(2)} €\n`;
          } else if (owner.name === "Patricia" && patriciaSharedExpense > 0) {
    ...
    <div className="relative">
      <button
        ...
        className={`px-3 py-1 rounded transition-colors ${
          showExportMenu ? "bg-orange-100 text-orange-700" : "hover:bg-gray-100"
        }`}
      >
